...
void trie_add_entry(struct trie *const trie, char const *str, void *data);
struct trie_search_result trie_search(const struct trie *const trie, char const *str);
struct trie_node *trie_level_search(const struct trie_node *const level, char const letter);
